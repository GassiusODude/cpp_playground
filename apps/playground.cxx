...
subscriber.setsockopt(5, TOPIC_A.c_str(), TOPIC_A.length());
std::cout << "Subscriber 1 connected\n";
...
//subscriber.setsockopt(zmq::socket_type::sub, zmq::str_buffer(""));
    std::cout << "Subscriber 2 connected\n";
    while (true) {
        // Receive all parts of the message
        std::vector<zmq::message_t> recv_msgs;
        zmq::recv_result_t result =
          zmq::recv_multipart(subscriber, std::back_inserter(recv_msgs));
        std::cout << "Subscriber 2\n";
//        assert(result && "recv failed");
        assert(*result == 2);
